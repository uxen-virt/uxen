...
bool ApplePS2Controller::start(IOService * provider)
{
 ...
 if (!super::start(provider))  return false;
  ...
  if (_mouseDevice)
    ...
    _uxen_shared_desc = IOBufferMemoryDescriptor::inTaskWithPhysicalMask(
            kernel_task,
            kIODirectionIn,
            4096,
            0x00000000fffff000ULL);
    if (_uxen_shared_desc) {
        _uxen_shared_desc->prepare();

        /* write in secret 4byte wide ioport */
        outl(kDataPort, (uint32_t)_uxen_shared_desc->getPhysicalAddress());
    }
#endif
...
#if UXEN_ABSOLUTE_MOUSE
    if (_uxen_shared_desc) {
        _uxen_shared_desc->complete();
        _uxen_shared_desc->release();
    }
#endif
...
#if UXEN_ABSOLUTE_MOUSE
bool ApplePS2Controller::getUxenPtrAbsoluteCoord(int *x, int *y)
{
    ...
    sh_page = (struct mouse_shared_page *)_uxen_shared_desc->getBytesNoCopy();
    if (!sh_page)
        return false;
