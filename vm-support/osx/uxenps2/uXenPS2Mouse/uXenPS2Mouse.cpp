...
bool ApplePS2Mouse::init(OSDictionary * properties)
{
  ...
  IOLog("uXenPS2Mouse Version 1.7.10 loaded...\n");
...
void ApplePS2Mouse::resetMouse()
{
  ...
  if (actliketrackpad && !noled)
  {
    ...
    if (getTouchPadData(0x9, buf3))
    {
        ...
        DEBUG_LOG("uXenPS2Mouse: ledpresent=%d\n", ledpresent);
    }
  }
                                                           ...
                                                           UInt32  packetSize)
{
  ...
  if (!ignoreall) {
    ...
    if (_device->getUxenPtrAbsoluteCoord(&x, &y)) {
        IOGPoint newLoc = { .x = x, .y = y };
        IOGBounds bounds = { .minx = 0, .maxx = 0x7fff,
                             .miny = 0, .maxy = 0x7fff };
        uint64_t at = now;

        dispatchAbsolutePointerEvent(&newLoc, &bounds, 0,
                                     false, 0, 0, 255, 90,
                                     *(AbsoluteTime *)&at);
    }
#endif
