...
void ApplePS2Keyboard::loadCustomPS2Map(OSDictionary* dict, const char* name)
{
    ...
    if (NULL != pArray)
    {
        for (int i = 0; i < pArray->getCount(); i++)
        {
            ...
            {
                IOLog("uXenPS2Keyboard: invalid custom PS2 map entry: \"%s\"\n", psz);
                continue;
            }
            ...
            {
                IOLog("uXenPS2Keyboard: scan code invalid for PS2 map entry: \"%s\"\n", psz);
                continue;
            }
...
void ApplePS2Keyboard::loadCustomADBMap(OSDictionary* dict, const char* name)
{
    ...
    if (NULL != pArray)
    {
        for (int i = 0; i < pArray->getCount(); i++)
        {
            ...
            {
                IOLog("uXenPS2Keyboard: invalid custom ADB map entry: \"%s\"\n", psz);
                continue;
            }
            ...
            {
                IOLog("uXenPS2Keyboard: scan code invalid for ADB map entry: \"%s\"\n", psz);
                continue;
            }
