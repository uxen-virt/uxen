...
BOOL bInitSURF(PPDEV ppdev, BOOL bFirst)
{
    ...
    if (bFirst || RemappingNeeded)
    {
        ...
        *(PULONG)(ppdev->pjScreen) = 0xaa55aa55;
        ...
        l = ppdev->cyScreen * (ppdev->lDeltaScreen > 0 ?
                               ppdev->lDeltaScreen :
                               -ppdev->lDeltaScreen);
        if (l > videoMemoryInformation.VideoRamLength) {
            DISPDBG((1, "Insufficient Video RAM\n"));
            return(FALSE);
        }

        RtlFillMemory(ppdev->pjScreen, l, 0xff);
        ...
        if (ppdev->pPointerAttributes)
            EngFreeMem(ppdev->pPointerAttributes);
        ppdev->pPointerAttributes = EngAllocMem(FL_ZERO_MEMORY,
                                                ppdev->cjPointerAttributes,
                                                ALLOC_TAG);
...
DEVINFO *pDevInfo)
{
    ...
    pGdiInfo->ulPhysicalPixelGamma = PPG_SRGB;
