...
PDRIVER_OBJECT  uxensfDriverObject;
                     ...
                     IN PUNICODE_STRING RegistryPath)
{
    ...
    uxensfDriverObject = DriverObject;
    ...
    switch (RxContext->MajorFunction)
    {
        ...
        case IRP_MJ_DEVICE_CONTROL:
        {
            switch (LowIoContext->ParamsFor.IoCtl.IoControlCode)
            {
                ...
                case IOCTL_MRX_VBOX_SERVERADDR:
                {
                    int len = LowIoContext->ParamsFor.IoCtl.InputBufferLength;
                    if (len >= sizeof(unsigned int))
                        uxensfServerAddr = *(unsigned int*)LowIoContext->ParamsFor.IoCtl.pInputBuffer;
                    Log(("VBOXSF: MRxDevFcbXXXControlFile: IOCTL_MRX_VBOX_SERVERADDR, addr=0x%x\n", uxensfServerAddr));
                    Status = STATUS_SUCCESS;
                    break;
                }
