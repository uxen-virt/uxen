...
CMiniportWaveCyclicUXenAudio::CMiniportWaveCyclicUXenAudio
(
    void
)
/*++
...
//=============================================================================
STDMETHODIMP
CMiniportWaveCyclicUXenAudio::GetDescription
(
...
Routine Description:
    ...
    The descriptor is defined in wavtable.h for each UXenAudio sample.
...
//=============================================================================
STDMETHODIMP
CMiniportWaveCyclicUXenAudio::Init
(
...
Arguments:
  ...
  ResourceList_ - resource list. UXenAudio does not use resources.
...
//=============================================================================
NTSTATUS
CMiniportWaveCyclicUXenAudio::PropertyHandlerCpuResources
(
...
//=============================================================================
NTSTATUS
CMiniportWaveCyclicUXenAudio::PropertyHandlerGeneric
(
    IN  PPCPROPERTY_REQUEST     PropertyRequest
)
/*++
...
//=============================================================================
NTSTATUS
CMiniportWaveCyclicUXenAudio::ValidateFormat
(
...
//-----------------------------------------------------------------------------
NTSTATUS
CMiniportWaveCyclicUXenAudio::ValidatePcm
(
...
CMiniportWaveCyclicStreamUXenAudio::CMiniportWaveCyclicStreamUXenAudio
(
    void
)
{
...
--*/
{
    ...
    if (m_pTimer)
    {
        ...
        ExFreePoolWithTag(m_pTimer, UXENAUDIO_POOLTAG);
    }

    if (m_pDpc)
    {
        ExFreePoolWithTag( m_pDpc, UXENAUDIO_POOLTAG );
    }
...
//=============================================================================
NTSTATUS
CMiniportWaveCyclicStreamUXenAudio::Init
(
    IN  PCMiniportWaveCyclicUXenAudio Miniport_,
    IN  ULONG                   Pin_,
    IN  BOOLEAN                 Capture_,
...
--*/
{
...
#else
	//ntStatus=m_AdapterCommon->VoiceSetFormat(DataFormat_);
#endif
    ...
    if (NT_SUCCESS(ntStatus))
    {
	m_pMiniport->m_AdapterCommon->VoiceRingSize(m_nVoiceNumber,&m_pMiniport->m_MaxDmaBufferSize);
...
//=============================================================================
// CMiniportWaveCyclicStreamUXenAudio IMiniportWaveCyclicStream
//=============================================================================

//=============================================================================
STDMETHODIMP
CMiniportWaveCyclicStreamUXenAudio::GetPosition
(
    OUT PULONG                  Position
)
/*++
...
--*/
{
    ...
    ntStatus=m_pMiniport->m_AdapterCommon->VoiceReadOffset(m_nVoiceNumber, &ret);
...
//=============================================================================
STDMETHODIMP
CMiniportWaveCyclicStreamUXenAudio::NormalizePhysicalPosition
(
...
CMiniportWaveCyclicStreamUXenAudio::SetFormat
(
...
--*/
{
    ...
    {
        // UXenAudio does not validate the format.
        //
        ...
        if (pWfx)
        {
            ...
            if (STATUS_SUCCESS == ntStatus)
            {
                if (!m_fCapture)
                {
		    // FIXME: XXX - JMM add suppport for other formats
                    //SetDataFormat(Format);
		    //m_AdapterCommon->VoiceSetFormat(DataFormat_);
                }
...
//=============================================================================
STDMETHODIMP_(ULONG)
CMiniportWaveCyclicStreamUXenAudio::SetNotificationFreq
(
    IN  ULONG                   Interval,
...
//=============================================================================
STDMETHODIMP
CMiniportWaveCyclicStreamUXenAudio::SetState
(
    IN  KSSTATE                 NewState
)
/*++
...
//=============================================================================
STDMETHODIMP_(void)
CMiniportWaveCyclicStreamUXenAudio::Silence
(
...
Routine Description:
  ...
  called whenever CMiniportWaveCyclicStreamUXenAudio::m_pTimer triggers.
...
--*/
{
    ...
    PCMiniportWaveCyclicUXenAudio pMiniport =
        (PCMiniportWaveCyclicUXenAudio) DeferredContext;

    pMiniport->m_AdapterCommon->VoiceNotify(0); //FIXME: currently only for debugging - but should enumurate voices 
