...
//=============================================================================
NTSTATUS
CreateMiniportWaveCyclicUXenAudio
( 
...
--*/
{
    ...
    return 
        CMiniportWaveCyclicUXenAudio::GetDescription(OutFilterDescriptor);
} // GetDescription
...
--*/
{
    ...
    if (NT_SUCCESS(ntStatus))
    {
        stream = new (NonPagedPool, UXENAUDIO_POOLTAG) 
    ...
    else
    {
        ...
        if (NT_SUCCESS(ntStatus))
        {
            if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET)
            {
                ...
                pComponentId->Product   = PID_UXENAUDIO;
                pComponentId->Name      = NAME_UXENAUDIO_SIMPLE;
                pComponentId->Component = GUID_NULL; // Not used for extended caps.
                pComponentId->Version   = UXENAUDIO_VERSION;
                pComponentId->Revision  = UXENAUDIO_REVISION;
