...
--*/
    ...
    if (NT_SUCCESS(ntStatus))
    {
        ...
        {
            ntStatus = 
                pAdapterCommon->Init(ResourceList, DeviceObject);
    ...
    // install UXenAudio topology miniport.
    //
    if (NT_SUCCESS(ntStatus))
    {
        ntStatus = 
    ...
    // install UXenAudio wavecyclic miniport.
    //
    if (NT_SUCCESS(ntStatus))
    {
        ntStatus = 
