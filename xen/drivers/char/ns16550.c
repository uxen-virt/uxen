 ...
 * by requesting irq 0.
 * uxen-addition: [-lb][-fc] to skip loopback/flowctrl init.
 */
static char __initdata opt_com1[36] = "";
static char __initdata opt_com2[36] = "";
...
static struct ns16550 {
    ...
    bool_t intr_works;
    bool_t loopback_skip;
    bool_t flowctrl_skip;
    /* PCI card parameters. */
...
static void ns16550_setup_preirq(struct ns16550 *uart)
{
    ...
    if (!uart->flowctrl_skip)
        /* No flow ctrl: DTR and RTS are both wedged high to keep
         * remote happy. */
    ...
    struct ns16550 *uart, const char *conf)
{
 ...
 config_parsed:
    while (*conf == '-') {
        conf++;
        if (!strncmp(conf, "lb", 2)) {
            uart->loopback_skip = 1;
            conf += 2;
        } else if (!strncmp(conf, "fc", 2)) {
            uart->flowctrl_skip = 1;
            conf += 2;
        }        
    }
    /* Sanity checks. */
...
void __init ns16550_init(int index, struct ns16550_defaults *defaults)
{
    ...
    uart->loopback_skip = 0;
    uart->flowctrl_skip = 0;
