...
{
    return (l->lock.recurse_cpu == smp_processor_id());
}
...
/* paging mark dirty vs read/clean log dirty info lock (per-p2m-table)
 * 
 * This protects marking pages dirty in the log dirty table
 * vs. reading/clearing the log dirty info and resetting the pages to
 * read-only.
 */

declare_mm_lock(logdirty)
#define logdirty_lock(p)           mm_lock(logdirty, &(p)->logdirty_lock)
#define logdirty_unlock(p)         mm_unlock(&(p)->logdirty_lock)
