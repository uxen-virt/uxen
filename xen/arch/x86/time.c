...
#define THIS_CPU_TIME &this_cpu(cpu_time)
#else   /* __UXEN__ */
...
{
    set_time_scale(&pmt_scale, ACPI_PM_FREQUENCY, MILLISECS(1000));
...
static void __init init_platform_timer(void)
{
    ...
    set_time_scale(&plt_scale, pts->frequency, MILLISECS(1000));
...
int cpu_frequency_change(u64 freq)
{
    ...
    set_time_scale(&t->tsc_scale, freq, MILLISECS(1000));
    local_irq_enable();
...
{
    int calibration_period = 1000;
    u64 tmp = init_pit_and_calibrate_tsc();

    set_time_scale(&this_cpu(cpu_time).tsc_scale, tmp,
                   MILLISECS(calibration_period));

    do_div(tmp, calibration_period);
...
static int _disable_pit_irq(void(*hpet_broadcast_setup)(void))
{
    ...
    if ( cpuidle_using_deep_cstate() && !boot_cpu_has(X86_FEATURE_ARAT) )
    {
        ...
        if ( !hpet_broadcast_is_available() )
        {
            if ( xen_cpuidle > 0 )
            {
                printk("%S() failed, turning to PIT broadcast\n",
                       (printk_symbol)hpet_broadcast_setup);
                return -1;
            }
            ret = 0;
        }
