...
void __dummy__(void)
{
    ...
    OFFSET(VCPU_softirq_pending, struct vcpu, softirq_pending);
    OFFSET(VCPU_fpu_dirtied, struct vcpu, fpu_dirtied);
    OFFSET(VCPU_fpu_ctxt, struct vcpu, arch.fpu_ctxt);
    OFFSET(VCPU_xsave_area, struct vcpu, arch.xsave_area);
    OFFSET(VCPU_xcr0, struct vcpu, arch.xcr0);
    OFFSET(VCPU_xcr0_accum, struct vcpu, arch.xcr0_accum);
    OFFSET(VCPU_xcr0_host, struct vcpu, arch.xcr0_accum);
    ...
    OFFSET(VCPU_svm_root_vmcb_pa, struct vcpu, arch.hvm_svm.root_vmcb_pa);
    BLANK();
    ...
    OFFSET(CPU_cr8, struct vcpu, arch.hvm_vcpu.hw_cr8);
    BLANK();
