    ...
    int top_word, uint32_t val)
{
    ...
    *pent = ent;
              ...
              hvm_irq->gsi_assert_count[idx] )
    {
        ...
        vioapic_deliver(vioapic, idx, VCPUID_ANY);
    }
                               ...
                               uint16_t vcpu_id)
{
    ...
    {
        vioapic_deliver(vioapic, irq, vcpu_id);
    }

    /* There's a subtle race condition here: *
     * Xen uses remote_irr (set here and cleared by eoi) *
     * to gate delivery of a second level interrupt. *
     * But it is possible to have a level interrupt clear *
     * and redeliver before EOI is called. In that case *
     * this logic will prevent that interrupt from ever *
     * working again. */
...
void vioapic_update_EOI(struct domain *d, int vector)
{
    ...
    for ( gsi = 0; gsi < VIOAPIC_NUM_PINS; gsi++ )
    {
        ...
        {
            ent->fields.remote_irr = 1;
            vioapic_deliver(vioapic, gsi, VCPUID_ANY);
        }
    }
...
int vioapic_init(struct domain *d)
{
    if (d->arch.hvm_domain.vioapic == NULL)
        d->arch.hvm_domain.vioapic = &d->extra_1->hvm_domain_vioapic;
