...
static int construct_vmcb(struct vcpu *v)
{
    ...
    arch_svm->msrpm = (unsigned long *)(uintptr_t)
        (v->domain->vm_info_shared->vmi_msrpm + (v->vcpu_id * MSRPM_SIZE));
    ...
    if ( paging_mode_hap(v->domain) )
    {
        ...
        vmcb->_g_pat = VMCB_DEFAULT_G_PAT;
        ...
        /* Even when using nested paging, we shadow the guest PAT */
        svm_enable_intercept_for_msr(v, MSR_IA32_CR_PAT);
    }
    else
    {
