...
static void __map_io_bitmap(struct vcpu *v, u64 vmcs_reg)
{
    ...
    if (nvmx->iobitmap[index])
        hvm_unmap_guest_frame_global (nvmx->iobitmap[index]);
    gpa = __get_vvmcs(vcpu_nestedhvm(v).nv_vvmcx, vmcs_reg);
    nvmx->iobitmap[index] = hvm_map_guest_frame_ro_global (gpa >> PAGE_SHIFT);
...
static void nvmx_purge_vvmcs(struct vcpu *v)
{
    ...
    if ( nvcpu->nv_vvmcxaddr != VMCX_EADDR )
        hvm_unmap_guest_frame_global(nvcpu->nv_vvmcx);
    nvcpu->nv_vvmcx == NULL;
    nvcpu->nv_vvmcxaddr = VMCX_EADDR;
    for (i=0; i<2; i++) {
        if ( nvmx->iobitmap[i] ) {
            hvm_unmap_guest_frame_global(nvmx->iobitmap[i]);
...
asmlinkage_abi void nvmx_switch_guest(void)
{
    struct vcpu *v = current;
    struct nestedvcpu *nvcpu = &vcpu_nestedhvm(v);
...
int nvmx_handle_vmptrld(struct cpu_user_regs *regs)
{
    ...
    if ( nvcpu->nv_vvmcxaddr == VMCX_EADDR )
    {
        nvcpu->nv_vvmcx = hvm_map_guest_frame_rw_global (gpa >> PAGE_SHIFT);
