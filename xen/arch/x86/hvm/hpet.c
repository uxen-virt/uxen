...
static int hpet_irq(HPETState *h, unsigned int tn)
{
...
#define set_update_timer(n)  (__set_bit((n), &update_timers))
    ...
    case HPET_T2_CMP:
        ...
        if ( hpet_enabled(h) && timer_enabled(h, tn) ) {
            if (!timer_is_periodic(h, tn) || h->pt[tn].one_shot)
                set_restart_timer(tn);
            else
                /*
                 * instead of recreating periodic timer, update
                 * the period and keep 1 leftover tick with old
                 * period. As per section 2.4.3.2 "Periodic Mode"
                 * of hpet specification, period updates should not
                 * be immediate but only take affect after pending
                 * comparator trigger */
                set_update_timer(tn);
        }
        break;
    ...
    while (update_timers)
    {
        i = find_first_set_bit(update_timers);
        __clear_bit(i, &update_timers);
        if ( (i == 0) && (h->hpet.config & HPET_CFG_LEGACY) )
        {
            /* HPET specification requires PIT shouldn't generate
             * interrupts if LegacyReplacementRoute is set for timer0 */
            pit_stop_channel0_irq(&vhpet_domain(h)->arch.vpit);
        }
        update_periodic_time(&h->pt[i], hpet_irq(h, i),
                             hpet_tick_to_ns(h, h->hpet.period[i]),
                             1);
    }
