...
static void assert_irq(struct domain *d, unsigned ioapic_gsi, unsigned pic_irq,
                       uint16_t vcpu_id)
{
    assert_gsi(d, ioapic_gsi, vcpu_id);
    ...
    struct domain *d, unsigned int isa_irq)
{
    ...
    if ( !__test_and_set_bit(isa_irq, &hvm_irq->isa_irq.i) &&
        ...
        assert_irq(d, gsi, isa_irq, VCPUID_ANY);
...
void hvm_isa_irq_assert_to_cpu(struct domain *d, unsigned int isa_irq,
                               uint16_t vcpu_id)
{
    struct hvm_irq *hvm_irq = &d->arch.hvm_domain.irq;
    unsigned int gsi = hvm_isa_irq_to_gsi(isa_irq);
    ...
    if (!__test_and_set_bit(isa_irq, &hvm_irq->isa_irq.i) &&
        (hvm_irq->gsi_assert_count[gsi]++ == 0))
        assert_irq(d, gsi, isa_irq, vcpu_id);
