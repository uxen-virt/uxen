...
#define X86_EFLAGS_SIF	0x80000000 /* shadow interrupt flag */
...
#define X86_CR4_PCID		0x20000 /* enable PCID */
...
#define PFEC_page_populate  (1U<<5)
...
static inline unsigned long read_cr4_cpu(void)
{
    unsigned long cr4;
...
int check_running_grace(void);

static always_inline void check_running(void)
{
    if (!_uxen_info.ui_running && check_running_grace())
        ABORT();
}
...
asmlinkage_sysv void do_ ## _name(struct cpu_user_regs *regs)
