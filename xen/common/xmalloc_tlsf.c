    ...
    unsigned long grow_size)
{
    ...
    pool = alloc_xenheap_pages(pool_order, 0);
    if ( pool == NULL )
        return NULL;
...
void *xmem_pool_alloc(unsigned long size, struct xmem_pool *pool)
{
    ...
    /* ensure fl is < REAL_FLI to avoid access outside sl_bitmap/matrix  */
    if (size >= ((1 << MAX_FLI) - ((1 << (MAX_FLI - 1 - MAX_LOG2_SLI)) - 1)))
        goto out;
...
static void *xmalloc_whole_pages(unsigned long size)
{
    struct bhdr *b;
    unsigned int pages =
        PAGE_ALIGN(size + BHDR_WHOLE_PAGE_OVERHEAD) >> PAGE_SHIFT;

    b = alloc_xenheap_pages(get_order_from_pages(pages), MEMF_xmalloc);
    if ( b == NULL )
        return NULL;

    b->size = pages << PAGE_SHIFT;
    return (void *)b->ptr.buffer;
}
...
void *_xmalloc(unsigned long size, unsigned long align)
{
    ...
    if ( size <= PAGE_SIZE - BHDR_OVERHEAD )
