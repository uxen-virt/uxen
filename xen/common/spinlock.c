...
static void check_lock(struct lock_debug *debug)
{
	...
	if (seen == !irq_safe)
	  printk("seen %u !irq_safe %u irq enabled %u\n", seen, !irq_safe,
...
#else
    check_barrier(&lock->debug);
    do { check_running(); mb(); } while ( _raw_spin_is_locked(&lock->raw) );
#endif
    mb();
}
...
#define CHECK_RUNNING_GRACE 0x1000000
static DEFINE_PER_CPU(uint32_t, check_running_grace_clock);

int check_running_grace(void) {

    if (this_cpu(check_running_grace_clock)++ > CHECK_RUNNING_GRACE) {
        this_cpu(check_running_grace_clock) = 0;
        return 1;
    }
    return 0;
}
