                          ...
                          uint64_t addr, uint64_t value)
{
    ...
    case 1 /* R_386_32 */ :
        u32 = ptr;
        *u32 += (uint32_t)(elf->reloc_offset + value +
                           elf->dest - (char *)NULL);
        break;
    case 2 /* R_386_PC32 */ :
        u32 = ptr;
        *u32 += (uint32_t)(elf->reloc_offset + value + elf->dest - (char *)u32);
        break;
    default:
        return -1;
    }
                            ...
                            uint64_t addr, uint64_t value)
{
    ...
    case 1 /* R_X86_64_64 */ :
        u64 = ptr;
        *u64 += (uint64_t)(elf->reloc_offset + value +
                           elf->dest - (char *)NULL);
        break;
...
static struct relocs {
    ...
    int (*func) (struct elf_binary * elf, int type, uint64_t addr,
                 uint64_t value);
} relocs[EM_X86_64 + 1] = { { NULL }, };
/* *INDENT-OFF* */
struct relocs reloc_EM_386 =
    /* [EM_386] = */ {
        /* .names = */ rel_names_i386,
        /* .count = */ sizeof(rel_names_i386) / sizeof(rel_names_i386[0]),
        /* .func = */ elf_reloc_i386,
    };
struct relocs reloc_EM_X86_64 =
    /* [EM_X86_64] = */ {
                             ...
                             const elf_shdr * sect, const elf_shdr * syms)
{
    ...
    for ( ; ptr < end; ptr += rsize )
    {
        ...
        case SHN_ABS:
            ...
            break;
        default:
            shdr = elf_shdr_by_index(elf, (int)shndx);
            if ( shdr == NULL )
            {
                elf_err(elf, "%s: invalid section: %" PRId64 "\n",
                        __FUNCTION__, shndx);
        ...
        addr = elf_uval(elf, sect, sh_addr) + r_offset;
...
int elf_reloc(struct elf_binary *elf)
{
    ...
    relocs[EM_386] = reloc_EM_386;
    relocs[EM_X86_64] = reloc_EM_X86_64;

    count = elf_shdr_count(elf);
    for ( i = 0; i < count; i++ )
    {
        rels = elf_shdr_by_index(elf, (int)i);
