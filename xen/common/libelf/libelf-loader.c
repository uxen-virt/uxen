...
int elf_init(struct elf_binary *elf, const char *image, size_t size)
{
    ...
    shdr = elf_shdr_by_index(elf, (int)section);
    if ( shdr != NULL )
    ...
    for ( i = 0; i < count; i++ )
    {
        shdr = elf_shdr_by_index(elf, (int)i);
...
do {                                            \
    ...
    maxva = (char *)(uintptr_t)elf_round_up(elf, (uintptr_t)maxva + sz);
    ...
    for ( i = 0; i < elf_shdr_count(elf); i++ )
    {
        ...
        if ( (type == SHT_STRTAB) || (type == SHT_SYMTAB) )
        {
             ...
             if (elf_64bit(elf))
                 shdr->e64.sh_offset = maxva - symtab_addr;
             else
                 shdr->e32.sh_offset = (uint32_t)(maxva - symtab_addr);
             maxva = (char *)(uintptr_t)elf_round_up(elf,
                                                     (uintptr_t)maxva + sz);
        }
        shdr = (elf_shdr *)((uintptr_t)shdr +
                            (uintptr_t)elf_uval(elf, elf->ehdr, e_shentsize));
    }

    /* Write down the actual sym size. */
    *(uint32_t *)symbase = (uint32_t)(maxva - symtab_addr);
...
void elf_parse_binary(struct elf_binary *elf)
{
    ...
    if ( count > 0 )
    {
        for ( i = 0; i < count; i++ )
        {
            phdr = elf_phdr_by_index(elf, (int)i);
        ...
        {
            shdr = elf_shdr_by_index(elf, (int)i);
            if ( !elf_shdr_is_loadable(elf, shdr) )
                continue;
            paddr = elf_uval(elf, shdr, sh_addr);
            memsz = elf_uval(elf, shdr, sh_size);
            elf_msg(elf, "%s: shdr: paddr=0x%" PRIx64
                    " memsz=0x%" PRIx64 "\n", __FUNCTION__, paddr, memsz);
            if ( low > paddr )
                low = paddr;
            if ( high < paddr + memsz )
    ...
    if ( count > 0 )
    {
        for ( i = 0; i < count; i++ )
        {
            ...
            elf_msg(elf, "%s: phdr %" PRIu64 " at 0x%p -> 0x%p\n",
                    __FUNCTION__, i, dest, dest + filesz);
        ...
        for ( i = 0; i < count; i++ )
        {
            ...
            if ( !elf_shdr_is_loadable(elf, shdr) )
                continue;
            paddr = elf_uval(elf, shdr, sh_addr);
            offset = elf_uval(elf, shdr, sh_offset);
            if ( elf_uval(elf, shdr, sh_type) != SHT_NOBITS )
                filesz = elf_uval(elf, shdr, sh_size);
            else
                filesz = 0;
            memsz = elf_uval(elf, shdr, sh_size);
            dest = elf_get_ptr(elf, paddr);
...
void *elf_get_ptr(struct elf_binary *elf, uintptr_t addr)
{
