...
int vsnprintf(char *buf, size_t size, const char *fmt, va_list args)
{
    ...
    repeat:
        ...
        case 'S': {             /* hijack %S to print a symbol */
            unsigned long addr, offset, size;
            char namebuf[KSYM_NAME_LEN + 1];
            const char *name;
            addr = (unsigned long)va_arg(args, void *);
            name = symbols_lookup(addr, &size, &offset, namebuf);
            if (!name) {
                /* symbol not found, print address as [<%p>] */
                if (str <= end)
                    *str = '[';
                ++str;
                if (str <= end)
                    *str = '<';
                ++str;
                str = number(str, end, addr, 16, 2 * sizeof(void *), -1,
                             SPECIAL);
                if (str <= end)
                    *str = '>';
                ++str;
            ...
            ++str;
            /* %#lx offset */
            str = number(str, end, offset, 16, -1, -1, SPECIAL);
            /* / */
            if (str <= end)
                *str = '/';
            ++str;
            /* %#lx size */
            str = number(str, end, size, 16, -1, -1, SPECIAL);
            continue;
        }
