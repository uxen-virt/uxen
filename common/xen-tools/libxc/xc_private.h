...
#if defined(_WIN32)
typedef CRITICAL_SECTION xc_critical_section;
#define xc_critical_section_init InitializeCriticalSection
#define xc_critical_section_enter EnterCriticalSection
#define xc_critical_section_leave LeaveCriticalSection
#define xc_critical_section_free DeleteCriticalSection
typedef void *xc_init_once_t;      /* XXX INIT_ONCE */
#define XC_INIT_ONCE_INIT NULL     /* XXX INIT_ONCE_STATIC_INIT */
#define xc_init_once(once_control, init_fn)                             \
    InitOnceExecuteOnce(once_control, (void *)init_fn, NULL, NULL)
typedef DWORD xc_tls_key;
#define xc_tls_key_create(pkey, cleanup) do {   \
        *(pkey) = TlsAlloc();                   \
        (void)(cleanup);                        \
    } while (0)
#define xc_tls_get(key) TlsGetValue(key)
#define xc_tls_set(key, val) TlsSetValue(key, val)
#elif defined(__APPLE__)
#include <pthread.h>
typedef pthread_mutex_t xc_critical_section;
void xc_critical_section_init(xc_critical_section *cs);
void xc_critical_section_enter(xc_critical_section *cs);
void xc_critical_section_leave(xc_critical_section *cs);
void xc_critical_section_free(xc_critical_section *cs);
typedef pthread_once_t xc_init_once_t;
#define XC_INIT_ONCE_INIT PTHREAD_ONCE_INIT
#define xc_init_once(once_control, init_fn)        \
    pthread_once(once_control, (void *)init_fn)
typedef pthread_key_t xc_tls_key;
#define xc_tls_key_create(pkey, cleanup) pthread_key_create(pkey, cleanup)
#define xc_tls_get(key) pthread_getspecific(key)
#define xc_tls_set(key, val) pthread_setspecific(key, val)
#else
#error implement me
#endif
...
#define HYPERCALL_BUFFER_CACHE_SIZE 4
    ...
    xc_critical_section hypercall_buffer_cache_mutex;
