...
uint64_t elf_access_unsigned(struct elf_binary * elf, const void *_ptr,
                             uint64_t offset, size_t size)
{
    ...
    case 1:
        ...
        return *u8;
    case 2:
        u16 = (uint16_t *)(ptr + offset);
        return need_swap ? bswap_16(*u16) : *u16;
    case 4:
        u32 = (uint32_t *)(ptr + offset);
        return need_swap ? bswap_32(*u32) : *u32;
    case 8:
        u64 = (uint64_t *)(ptr + offset);
...
int64_t elf_access_signed(struct elf_binary *elf, const void *_ptr,
                          uint64_t offset, size_t size)
{
    int need_swap = elf_swap(elf);
    const int8_t *ptr = _ptr;
    const int8_t *s8;
    const int16_t *s16;
    const int32_t *s32;
    const int64_t *s64;
    ...
    case 1:
        ...
        return *s8;
    case 2:
        s16 = (int16_t *)(ptr + offset);
        return need_swap ? bswap_16(*s16) : *s16;
    case 4:
        s32 = (int32_t *)(ptr + offset);
        return need_swap ? bswap_32(*s32) : *s32;
    case 8:
        s64 = (int64_t *)(ptr + offset);
...
{
    const uint8_t *ptr = elf_section_start(elf, elf->sym_tab);
    const uint8_t *end = elf_section_end(elf, elf->sym_tab);
    const elf_sym *sym;
    uint64_t info, name;

    for ( ; ptr < end; ptr += elf_size(elf, sym) )
    {
        sym = (const elf_sym *)ptr;
...
const elf_sym *elf_sym_by_index(struct elf_binary *elf, int index)
{
    ...
    sym = (const elf_sym *)(ptr + index * elf_size(elf, sym));
    return sym;
}

const char *elf_note_name(struct elf_binary *elf, const elf_note * note)
{
    return (char *)((uint8_t *)note + elf_size(elf, note));
}
...
const void *elf_note_desc(struct elf_binary *elf, const elf_note * note)
{
    ...
    return (uint8_t *)note + elf_size(elf, note) + namesz;
}
...
const elf_note *elf_note_next(struct elf_binary *elf, const elf_note * note)
{
    ...
    return (elf_note *)((uint8_t *)note + elf_size(elf, note) +
                        namesz + descsz);
}
...
int elf_shdr_is_loadable(struct elf_binary *elf, const elf_shdr * shdr)
{
    uint64_t s_flags = elf_uval(elf, shdr, sh_flags);

    return ((s_flags & SHF_ALLOC) != 0);
}
