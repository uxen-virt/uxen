...
uxen_op_create_vm(struct uxen_createvm_desc *ucd, struct fd_assoc *fda)
{
    ...
    if (fda->vmi) {
        fail_msg("domain %d: %" PRIuuid " vmi already exists",
                 ucd->ucd_domid, PRIuuid_arg(ucd->ucd_vmuuid));
        return -EEXIST;
    }
    ...
    if (ucd->ucd_max_vcpus > UXEN_MAX_VCPUS) {
...
#define EVENT_WAIT(object, interruptible, timeout)                      \
    ...
    while (is_vci_runnable(vci)) {
        ...
        uxen_cpu_pin_vcpu(vci, KeGetCurrentProcessorNumber());
	...
	uxen_cpu_unpin_vcpu(vci);
        ...
        case VCI_RUN_MODE_YIELD: {
            ...
            if (num_executing == 0 &&
                num_halt < vmi->vmi_shared.vmi_nrvcpus-1)
