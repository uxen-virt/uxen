...
HFSCatalogNodeID newFolder(const char* pathName, Volume* volume,
	...
	HFSPlusCatalogFile *newFileInFolder(const char* pathName, Volume* volume, HFSPlusCatalogFolder* parentFolder, int mode, uint16_t uid, uint16_t gid, uint32_t rdev);
    int storeNewFile(const char* name, HFSPlusCatalogFile *file, Volume *volume, HFSPlusCatalogFolder *parentFolder);
