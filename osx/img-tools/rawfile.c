	...
	if(blocksNeeded > forkData->totalBlocks) {
		...
		while(blocksToAllocate > 0) {
			if(isBlockUsed(volume, curBlock)) {
                ...
                /* XXX this does not work past 4GB of used space, because of an
                 * integer overflow in the multiplication.  Instead the write
                 * will corrupt B-trees at beginning of the volume. Anyway we
                 * don't care to zero the sectors we allocate.
                 */
