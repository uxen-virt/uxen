...
int removeFile(const char* fileName, const bool ignore_missing_files, Volume* volume) {
  HFSPlusCatalogRecord* record;
  HFSPlusCatalogKey key;
  io_func* io;
...
HFSCatalogNodeID newFolder(const char* pathName, Volume* volume,
        int mode, uint16_t uid, uint16_t gid) {
  HFSPlusCatalogFolder* parentFolder;
  HFSPlusCatalogFolder folder;
  HFSPlusCatalogKey key;
  HFSPlusCatalogThread thread;
  ...
  folder.permissions.ownerID = uid ? uid : parentFolder->permissions.ownerID;
  folder.permissions.groupID = gid ? gid : parentFolder->permissions.groupID;
  folder.permissions.adminFlags = 0;
  folder.permissions.ownerFlags = 0;
  folder.permissions.fileMode = mode ? mode : S_IFDIR | S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH;
...
HFSPlusCatalogFile *newFileInFolder(const char* name, Volume* volume,
  HFSPlusCatalogFolder* parentFolder, int mode, uint16_t uid, uint16_t gid, uint32_t rdev)
{
    HFSPlusCatalogFile *file = malloc(sizeof(HFSPlusCatalogFile));
    memset(file, 0, sizeof(*file));
    HFSPlusCatalogFile flippedFile;
    assert(file);
    HFSPlusCatalogKey key;
    HFSPlusCatalogThread thread;
...
//#error "set real date?"
    ...
    file->permissions.ownerID = uid;
    file->permissions.groupID = gid;
    ...
    //file->permissions.fileMode = (file->permissions.fileMode & 0770000) | mode;
    file->permissions.fileMode = mode;
    file->permissions.special.rawDevice = rdev;
    ...
    memcpy(&flippedFile, file, sizeof(flippedFile));
    flipCatalogFile(&flippedFile);

    ASSERT(addToBTree(volume->catalogTree, (BTKey*)(&key), sizeof(HFSPlusCatalogFile), (unsigned char *)(&flippedFile)), "addToBTree");
    key.nodeName.length = 0;
    ...
    /* Return non-endian flipped version of file... */
    return file;
}
...
int chmodFileByRecord(HFSPlusCatalogRecord* record, int mode, bool recurse, Volume* volume) {
  ...
  if(record->recordType == kHFSPlusFolderRecord) {
    ...
    if (recurse == true) {
      ...
      while(list != NULL) {
        chmodFileByRecord(list->record, mode, recurse, volume);      
        list = list->next;
      }
...
int chmodFile(const char* pathName, int mode, bool recurse, Volume* volume) {
  HFSPlusCatalogRecord* record;
   
  record = getRecordFromPath(pathName, volume, NULL, NULL);
 
  int rv =  chmodFileByRecord(record, mode, recurse, volume);
...
int chownFileByRecord(HFSPlusCatalogRecord* record, uint32_t owner, uint32_t group, bool recurse, Volume* volume) {
  ...
  if(record->recordType == kHFSPlusFolderRecord) {
    ...
    if (recurse == true) {
      theList = list = getFolderContents(((HFSPlusCatalogFolder*)record)->folderID, volume);  
      while(list != NULL) {
        chownFileByRecord(list->record, owner, group, recurse, volume);      
        list = list->next;
      }
      releaseCatalogRecordList(theList);
    }
...
int chownFile(const char* pathName, uint32_t owner, uint32_t group, bool recurse, Volume* volume) {
  HFSPlusCatalogRecord* record;
   
  record = getRecordFromPath(pathName, volume, NULL, NULL);
  
  int rv =  chownFileByRecord(record, owner, group, recurse, volume);
