...
void writeToFile(HFSPlusCatalogFile* file, AbstractFile* output, Volume* volume) {
	...
	/* Code assumed to be non-reentrant and single-threaded. */
	if (!buffer) buffer = (unsigned char*)malloc(BUFSIZE);
...
void removeAllInFolder(HFSCatalogNodeID folderID, Volume* volume, const char* parentName) {
	...
	while(list != NULL) {
		...
		if(list->record->recordType == kHFSPlusFolderRecord) {
			...
			printf("%s\n", fullName);
			removeFile(fullName, FALSE, volume);
		}
...
void addAllInFolder(HFSCatalogNodeID folderID, Volume* volume, const char* parentName) {
	...
	while((ent = readdir(dir)) != NULL) {
		...
		if((tmp = opendir(ent->d_name)) != NULL) {
			...
			if(cnid == 0) {
				cnid = newFolder(fullName, volume, 0, 0, 0);
			}
			...
			if(strncmp(fullName, "/Applications/", sizeof("/Applications/") - 1) == 0) {
				...
				if(strcmp(testBuffer, fullName) == 0) {
					...
					|| strcmp(ent->d_name, "BootNeuter") == 0
						...
						chmodFile(fullName, 04755, false, volume);
					} else {
						printf("Giving permissions to %s\n", fullName); fflush(stdout);
						chmodFile(fullName, 0755, false, volume);
					}
				}
...
static void displayFileLSLine2(HFSPlusCatalogFile* file, const char* name) {
	...
	printf("%3d ", file->permissions.ownerID);
	printf("%3d ", file->permissions.groupID);
 	printf("rd=0x%x ", file->permissions.special.rawDevice);
...
void hfs_ls(Volume* volume, const char* path) {
	...
	if(record != NULL) {
		if(record->recordType == kHFSPlusFolderRecord)
			...
			displayFileLSLine2((HFSPlusCatalogFile*)record, name);
	} else {
		printf("No such file or directory\n");
	}

	printf("Total filesystem size: %zu, free: %zu\n", ((size_t)volume->volumeHeader->totalBlocks - (size_t)volume->volumeHeader->freeBlocks) * (size_t)volume->volumeHeader->blockSize, (size_t)volume->volumeHeader->freeBlocks * (size_t)volume->volumeHeader->blockSize);
...
static void hfs_find2(Volume* volume, const char* path, int depth) {
	...
	while(list != NULL) {
        ...
        const char *uni = unicodeToAscii(&list->name);
        char fullName[1024];
        if (strcmp(path, "/") == 0) {
            strcpy(fullName, uni);
        } else {
            sprintf(fullName, "%s/%s", path, uni);
        }

		if(strlen(uni) && list->record->recordType == kHFSPlusFolderRecord) {
            printf("%s:\n", fullName);
            hfs_find2(volume, fullName, depth + 1);
		} else {
            displayFileLSLine2((HFSPlusCatalogFile*)(list->record), fullName);
            //printf("%s\n", fullName);
        }
...
void hfs_find(Volume* volume, const char* path) {
    return hfs_find2(volume, path, 0);
}
	...
	while(curRecord < tarSize) {
		...
		if(record) {
			if(record->recordType == kHFSPlusFolderRecord || type == 5) {
				...
				printf("replacing %s\n", fileName);
				free(record);
				removeFile(fileName, FALSE, volume);
			}
		}
		...
		if(type == 0) {
			...
			newFolder(fileName, volume, 0, 0, 0);
		} else if(type == 2) {
			printf("symlink: %s (%04o) -> %s\n", fileName, mode, target);
			makeSymlink(fileName, target, volume);
		}

		chmodFile(fileName, mode, false, volume);
		chownFile(fileName, uid, gid, false, volume);
