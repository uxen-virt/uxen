...
void cmd_chmod(Volume* volume, bool recurse, int argc, const char *argv[]) {
	...
	if(argc > 2) {
		...
		chmodFile(argv[2], mode, recurse, volume);
	} else {
		printf("Not enough arguments");
	}
}
...
void cmd_chown(Volume* volume, bool recurse, int argc, const char *argv[]) {
	...
	if(argc > 3) {
		...
		chownFile(argv[3], user_id, group_id, recurse, volume);
	} else {
		printf("Not enough arguments");
	}
}
...
int main(int argc, const char *argv[]) {
	...
	if(argc > 1) {
		if(strcmp(argv[2], "ls") == 0) {
			...
			cmd_chmod(volume, false, argc - 2, argv + 2);
		} else if(strcmp(argv[2], "chown") == 0) {
			cmd_chown(volume, false, argc - 2, argv + 2);
		} else if(strcmp(argv[2], "chmodr") == 0) {
			cmd_chmod(volume, true, argc - 2, argv + 2);
		} else if(strcmp(argv[2], "chownr") == 0) {
			cmd_chown(volume, true, argc - 2, argv + 2);
