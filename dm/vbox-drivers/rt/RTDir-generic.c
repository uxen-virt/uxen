                            ...
                            PCRTTIMESPEC pChangeTime, PCRTTIMESPEC pBirthTime)
{
    ...
    return RTPathSetTimesUcs(pDir->pwszPath, pAccessTime, pModificationTime, pChangeTime, pBirthTime);
}

RTR3DECL(int) RTDirQueryInfo(PRTDIR pDir, PRTFSOBJINFO pObjInfo, RTFSOBJATTRADD
enmAdditionalAttribs)
{
        ...
        */
        if (!rtDirValidHandle(pDir))
            return VERR_INVALID_PARAMETER;
        return RTPathQueryInfoExUcs(pDir->pwszPath, pObjInfo, 
            enmAdditionalAttribs, RTPATH_F_FOLLOW_LINK);
}
