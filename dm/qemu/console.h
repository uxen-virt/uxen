...
extern int gui_refresh_interval;
...
extern struct DisplayAllocator display_allocator;
...
static inline void dpy_cursor_shape(struct DisplayState *s,
                                    int w, int h,
                                    int hot_x, int hot_y,
                                    uint8_t *mask, uint8_t *color) {
    struct DisplayChangeListener *dcl = s->listeners;
    while (dcl != NULL) {
        if (dcl->dpy_cursor_shape)
            dcl->dpy_cursor_shape(s, w, h, hot_x, hot_y, mask, color);
        dcl = dcl->next;
    }
}
...
void sdl_handle_event(DisplayState *ds);
