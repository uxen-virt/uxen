...
IDEDevice *ide_create_drive(IDEBus *bus, int unit, DriveInfo *drive)
{
    ...
    if (drive->version)
        qdev_prop_set_string(dev, "ver", drive->version);
    if (drive->serial)
        qdev_prop_set_string(dev, "serial", drive->serial);
    if (drive->model)
        qdev_prop_set_string(dev, "model", drive->model);
...
static int ide_dev_initfn(IDEDevice *dev, IDEDriveKind kind)
{
    ...
    if (!dev->serial) {
        dev->serial = g_strdup(s->drive_serial_buf);
    }
