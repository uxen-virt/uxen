...
do { debug_printf("ahci: %s: [%d] ", __FUNCTION__, port); \
     debug_printf(fmt, ## __VA_ARGS__); } while (0)
#else
#define DPRINTF(port, fmt, ...) do {} while(0)
#endif

#define cur_cmd(ad) (&((AHCICmdHdr *)(ad)->lst)[(ad)->cur_slot])
		     ...
		     uint32_t wanted)
{
    ...
    *ptr = vm_memory_map(addr, &len, 1, 0);
    if (!*ptr) {
        error_printf("%s: vm_memory_map(addr) failed\n", __FUNCTION__);
        return;
    }
    if (len < wanted) {
        error_printf("%s: vm_memory_map(addr) short\n", __FUNCTION__);
        vm_memory_unmap(addr, len, 1, 0, *ptr, len);
        *ptr = NULL;
    }
}
    ...
    switch (offset) {
        case PORT_LST_ADDR:
            map_page(&s->dev[port].lst,
                     ((uint64_t)pr->lst_addr_hi << 32) | val,
                     ((uint64_t)pr->lst_addr_hi << 32) | pr->lst_addr,
		     1024);
            pr->lst_addr = val;
            s->dev[port].cur_slot = -1;
            break;
        case PORT_LST_ADDR_HI:
            map_page(&s->dev[port].lst,
                     ((uint64_t)val << 32) | pr->lst_addr,
                     ((uint64_t)pr->lst_addr_hi << 32) | pr->lst_addr,
		     1024);
            pr->lst_addr_hi = val;
            s->dev[port].cur_slot = -1;
        ...
        case PORT_FIS_ADDR:
            map_page(&s->dev[port].res_fis,
                     ((uint64_t)pr->fis_addr_hi << 32) | val,
                           ...
                           uint64_t val, unsigned size)
{
    ...
    if (addr & 3) {
        error_printf("ahci: Mis-aligned write to addr 0x%" PRIx64 "\n",
                     addr);
        return;
    }
                                 ...
                                 QEMUSGList *sglist)
{
    ...
    for (i = 0; len && sgcount; i++) {
        ...
        vm_memory_rw(cur_prd->base, buffer, once, 0);
                                ...
                                QEMUSGList *sglist)
{
    ...
    for (i = 0; len && sgcount; i++) {
        ...
        vm_memory_rw(cur_prd->base, buffer, once, 1);
        if (cur_prd->completion_cb)
            cur_prd->completion_cb(cur_prd, cur_prd->opaque);
        cur_prd++;
        sgcount--;
...
static void ahci_reset_port(AHCIState *s, int port)
{
    ...
    s->dev[port].port_regs.cmd &= ~(PORT_CMD_FIS_ON|PORT_CMD_LIST_ON);
    if (!ide_state->bs) {
...
static void ahci_write_fis_d2h(AHCIDevice *ad, uint8_t *cmd_fis)
{
    ...
    if (!cmd_fis) {
        /* map cmd_fis */
        tbl_addr = le64_to_cpu(cur_cmd(ad)->tbl_addr);
        cmd_fis = vm_memory_map(tbl_addr, &cmd_len, 0, 0);
	if (cmd_len != 0x80) {
            error_printf("%s: vm_memory_map(tbl_addr) short", __FUNCTION__);
            goto out;
        }
    }
...
{
    AHCIDevice *ad = (void *)opaque;

    DPRINTF(ad->port_no, "\n");

    ahci_trigger_irq(ad->hba, ad, PORT_IRQ_STAT_DPS);
}

static int ahci_populate_sglist(AHCIDevice *ad, QEMUSGList *sglist)
{
    AHCICmdHdr *cmd = cur_cmd(ad);
	...
	uint64_t addr;
	uint32_t len;
	SGEntryCompletion cb = NULL;

	vm_memory_rw_direct(prdt_addr + i * sizeof(AHCI_SG), (uint8_t *)&tbl,
			    sizeof(AHCI_SG), 0);
	...
	qemu_sglist_add_completion(sglist, addr, len + 1, cb, ad);
    }
                                ...
                                int slot)
{
    ...
    if (ncq_tfs->used) {
        ...
        error_printf("%s: tag %d already used\n", __FUNCTION__, tag);
        return;
    }
...
static int handle_cmd(AHCIState *s, int port, int slot)
{
    ...
    if (cmd_fis[1] == SATA_FIS_REG_H2D_UPDATE_COMMAND_REGISTER) {
        ...
        if (ide_state->drive_kind != IDE_CD) {
             ...
             */
            /* get ATA_CB_DH_LBA from VM provided select field */
            ide_state->select = cmd_fis[7] & 0xf0;
            switch (cmd_fis[2]) {
            case WIN_VERIFY_EXT:
            case WIN_READ_EXT:
            case WIN_WRITE_EXT:
...
static int ahci_start_transfer(IDEDMA *dma)
{
    ...
    /* write == ram -> device */
    uint32_t opts = le32_to_cpu(cur_cmd(ad)->opts);
    ...
    cur_cmd(ad)->status = cpu_to_le32(le32_to_cpu(cur_cmd(ad)->status) + size);
...
static int ahci_dma_retire_buf(IDEDMA *dma, int is_write)
{
    AHCIDevice *ad = DO_UPCAST(AHCIDevice, dma, dma);
    IDEState *s = &ad->port.ifs[0];

    cur_cmd(ad)->status = cpu_to_le32(le32_to_cpu(cur_cmd(ad)->status) +
                                      s->io_buffer_size);
...
static int ahci_dma_rw_buf(IDEDMA *dma, int is_write)
{
    ...
    /* update number of transferred bytes */
    cur_cmd(ad)->status = cpu_to_le32(le32_to_cpu(cur_cmd(ad)->status) + l);
    s->io_buffer_index += l;
...
static int ahci_dma_is_active(IDEDMA *dma)
{
    AHCIDevice *ad = DO_UPCAST(AHCIDevice, dma, dma);

    return ad->dma_cb != NULL;
}
...
static const IDEDMAOps ahci_dma_ops = {
    ...
    .retire_buf = ahci_dma_retire_buf,
    .rw_buf = ahci_dma_rw_buf,
    .set_unit = ahci_dma_set_unit,
    .add_status = ahci_dma_add_status,
    .set_inactive = ahci_dma_set_inactive,
    .is_active = ahci_dma_is_active,
...
void ahci_create_devs(struct AHCIPCIState *d, DriveInfo **hd_table)
{
    AHCIState *s = &d->ahci;
    int i;
    ...
    for (i = 0; i < s->ports; i++) {
        if (hd_table[i] == NULL)
            continue;
        ide_create_drive(&s->dev[i].port, 0, hd_table[i]);
    }
}

const VMStateDescription vmstate_ahci_port_regs = {
    .name = "ahci-port-regs",
    .version_id = 3,
    .minimum_version_id = 0,
    .minimum_version_id_old = 0,
    .fields = (VMStateField []) {
	VMSTATE_UINT32(lst_addr, AHCIPortRegs),
	VMSTATE_UINT32(lst_addr_hi, AHCIPortRegs),
	VMSTATE_UINT32(fis_addr, AHCIPortRegs),
	VMSTATE_UINT32(fis_addr_hi, AHCIPortRegs),
	VMSTATE_UINT32(irq_stat, AHCIPortRegs),
	VMSTATE_UINT32(irq_mask, AHCIPortRegs),
	VMSTATE_UINT32(cmd, AHCIPortRegs),
	VMSTATE_UINT32(unused0, AHCIPortRegs),
	VMSTATE_UINT32(tfdata, AHCIPortRegs),
	VMSTATE_UINT32(sig, AHCIPortRegs),
	VMSTATE_UINT32(scr_stat, AHCIPortRegs),
	VMSTATE_UINT32(scr_ctl, AHCIPortRegs),
	VMSTATE_UINT32(scr_err, AHCIPortRegs),
	VMSTATE_UINT32(scr_act, AHCIPortRegs),
	VMSTATE_UINT32(cmd_issue, AHCIPortRegs),
	VMSTATE_UINT32(reserved, AHCIPortRegs),
	VMSTATE_END_OF_LIST()
    }
};
...
static int ahci_device_post_load(void *opaque, int version_id)
{
    ...
    map_page(&ad->lst, ((uint64_t)pr->lst_addr_hi << 32) | pr->lst_addr,
	     ((uint64_t)pr->lst_addr_hi << 32) | pr->lst_addr, 1024);
    if (!ad->lst)
        return -ENOMEM;
    map_page(&ad->res_fis, ((uint64_t)pr->fis_addr_hi << 32) | pr->fis_addr,
...
const VMStateDescription vmstate_ahci_device = {
	...
	VMSTATE_STRUCT(port_regs, AHCIDevice, 0, vmstate_ahci_port_regs,
		       AHCIPortRegs),
	/* dma_status */
	VMSTATE_INT32(done_atapi_packet, AHCIDevice),
	VMSTATE_INT32(busy_slot, AHCIDevice),
	VMSTATE_INT32(init_d2h_sent, AHCIDevice),
	VMSTATE_INT32(cur_slot, AHCIDevice),
	/* ncq_tfs */
	VMSTATE_END_OF_LIST()
    }
};

const VMStateDescription vmstate_ahci_control_regs = {
    .name = "ahci-control-regs",
    .version_id = 3,
    .minimum_version_id = 0,
    .minimum_version_id_old = 0,
    .fields = (VMStateField []) {
	VMSTATE_UINT32(ghc, AHCIControlRegs),
	VMSTATE_UINT32(irqstatus, AHCIControlRegs),
	VMSTATE_END_OF_LIST()
    }
};
...
const VMStateDescription vmstate_ahci = {
	...
	VMSTATE_STRUCT(control_regs, AHCIState, 0, vmstate_ahci_control_regs,
		       AHCIControlRegs),
