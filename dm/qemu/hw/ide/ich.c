...
static const VMStateDescription vmstate_ich = {
    .name = "ich",
    .version_id = 3,
    .minimum_version_id = 0,
    .minimum_version_id_old = 0,
    // .post_load = ich_post_load,
    .fields = (VMStateField []) {
	VMSTATE_PCI_DEVICE(card, AHCIPCIState),
	VMSTATE_STRUCT(ahci, AHCIPCIState, 0, vmstate_ahci, AHCIState),
	VMSTATE_END_OF_LIST()
    }
};
...
PCIDevice *pci_ich_ide_init(PCIBus *bus, DriveInfo **hd_table, int devfn)
{
    PCIDevice *dev;
    struct AHCIPCIState *d;

    dev = pci_create_simple(bus, devfn, "ich9-ahci");
    d = DO_UPCAST(struct AHCIPCIState, card, dev);
    ahci_create_devs(d, hd_table);
    return dev;
}
