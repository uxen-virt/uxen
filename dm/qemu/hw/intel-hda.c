...
static void intel_hda_corb_run(IntelHDAState *d)
{
    ...
    for (;;) {
        ...
        verb = le32_to_cpu(vm_memory_read(addr + 4*rp, 4));
        d->corb_rp = rp;
...
static void intel_hda_response(HDACodecDevice *dev, bool solicited, uint32_t response)
{
    ...
    vm_memory_write(addr + 8*wp, cpu_to_le32(response), 4);
    vm_memory_write(addr + 8*wp + 4, cpu_to_le32(ex), 4);
    d->rirb_wp = wp;
                           ...
                           uint8_t *buf, uint32_t *len)
{
	...
	*len = st->fifos + 1;
    ...
    if (d->dp_lbase & 0x01) {
        ...
        vm_memory_write(addr + 8*s, cpu_to_le32(st->lpib), 4);
    }
    dprint(d, 3, "dma: --\n");
    ...
    if (irq) {
        ...
        intel_hda_update_irq(d);
#if 0
	debug_printf("I %05"PRId64" -\n", os_get_clock_ms() % 100000);
#endif
    }
    return true;
}
...
#define HDA_STREAM(_t, _i)                                            \
    ...
    [ ST_REG(_i, ICH6_REG_SD_FIFOSIZE) ] = {                          \
        ...
        .size     = 2,                                                \
        .reset    = _i < 4 ? 0x77 /* HDA_SDINFIFO_120B */ : 0xbf /* HDA_SDONFIFO_192B */, \
	.wmask    = 0x00ff,					      \
	.offset   = offsetof(IntelHDAState, st[_i].fifos),	      \
    },                                                                \
    [ ST_REG(_i, ICH6_REG_SD_FIFOW) ] = {			      \
        .stream   = _i,                                               \
        .name     = _t stringify(_i) " FIFOW",                        \
        .size     = 2,                                                \
        .reset    = _i < 4 ? 0x2 /* HDA_SDFIFOW_8B */ : 0x4 /* HDA_SDFIFOW_32B */, \
	.wmask    = 0x0007,					      \
	.offset   = offsetof(IntelHDAState, st[_i].fifow),	      \
    },                                                                \
...
extern CRITICAL_SECTION audio_cs;
				 ...
				 uint64_t val, unsigned size)
{
    ...
    intel_hda_reg_write(d, reg, val, ((uint64_t)1 << (size * 8)) - 1);
				    ...
				    unsigned size)
{
    ...
    EnterCriticalSection(&audio_cs);
    ret = intel_hda_reg_read(d, reg, ((uint64_t)1 << (size * 8)) - 1);
    LeaveCriticalSection(&audio_cs);
...
#ifdef AUDIO_MT
    audio_is = ioreq_new_server();
    memory_region_set_serverid(&d->mmio, audio_is->serverid);
#endif	/* AUDIO_MT */
...
static const VMStateDescription vmstate_intel_hda_stream = {
	...
	// VMSTATE_UINT32(fifos, IntelHDAStream),
	// VMSTATE_UINT32(fifow, IntelHDAStream),
        VMSTATE_END_OF_LIST()
    }
};
...
static PCIDeviceInfo intel_hda_info = {
    ...
    .qdev.props   = (Property[]) {
        DEFINE_PROP_UINT32("debug", IntelHDAState, debug, 1),
...
int intel_hda_and_codec_init(PCIBus *bus)
{
    ...
    BusState *hdabus;
    DeviceState *codec;
    const char *mode = "hda-output";

    if (vm_audio) {
        const char *m;

        if (dict_get_boolean(vm_audio, "disabled"))
            return 0;

        m = dict_get_string(vm_audio, "mode");
        if (m)
            mode = m;
    }
