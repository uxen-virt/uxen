...
static void hda_audio_output_cb(void *opaque, int avail)
{
    ...
    while (avail - sent >= st->bsize) {
        if (st->bpos >= st->bsize) {
	    st->bsize = sizeof(st->buf);
...
#if 0
    debug_printf("C %05"PRId64" - hda_audio_output_cb: avail %d sent %d\n",
                 os_get_clock_ms() % 100000, avail / 4, sent / 4);
#endif
}
...
static const VMStateDescription vmstate_hda_audio_stream = {
    ...
    .fields = (VMStateField []) {
        ...
        // VMSTATE_UINT32(bsize, HDAAudioStream),
