...
{
    const wchar_t *str = pString->String.ucs2;
    ...
    if (RT_UNLIKELY(str[pString->u16Length/2]))
        return false;
    return true;
}
...
#define SHFL_CF_ACCESS_MASK_DENY        (SHFL_CF_ACCESS_DENYREAD | SHFL_CF_ACCESS_DENYWRITE | SHFL_CF_ACCESS_DENYDELETE)
...
/** Do not allow delete. */
#define SHFL_CF_ACCESS_DENYDELETE       (0x00080000)
/** Do not allow access. */
#define SHFL_CF_ACCESS_DENYALL          (SHFL_CF_ACCESS_DENYREAD | SHFL_CF_ACCESS_DENYWRITE | SHFL_CF_ACCESS_DENYDELETE)
...
#define SHFL_ADD_MAPPING_F_CRYPT            (RT_BIT_32(3))
